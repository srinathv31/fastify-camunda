...
describe('onboard-user handlers (validate, background check, onboarding)', () => {
  ...
  it('completes validate-user-information on success', async () => {
    ...
    const task = makeTask({ userId: 'Alice', applicationId: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa' });
    const taskService = makeTaskService();
    await client.__trigger('onboard-user.validate-user-information', { task, taskService });
    ...
    expect(outVars).toEqual({ validated: true, normalizedUserId: 'alice' });
    ...
    expect(eventSpy).toHaveBeenCalledWith(expect.objectContaining({ result: 'success' }));
  });

  it('reports BPMN error when validate-user-information fails', async () => {
    const eventSpy = jest.spyOn(app, 'eventLog').mockResolvedValue(undefined);
    const task = makeTask({ userId: 'invalid_user', applicationId: 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb' });
    const taskService = makeTaskService();
    await client.__trigger('onboard-user.validate-user-information', { task, taskService });
    ...
    expect(eventSpy).toHaveBeenCalledWith(expect.objectContaining({ result: 'bpmn_error' }));
  });
  ...
  it('runs background check and completes on success', async () => {
    ...
    (http.get as jest.Mock).mockResolvedValueOnce({ body: { passed: true, score: 42 } });
    const task = makeTask({ userId: 'userB', validated: true, applicationId: 'cccccccc-cccc-cccc-cccc-cccccccccccc' });
    const taskService = makeTaskService();
    await client.__trigger('onboard-user.run-background-check', { task, taskService });
    expect(taskService.complete).toHaveBeenCalledTimes(1);
    const outVars = (taskService.complete.mock.calls[0][1] as any).getAll();
    expect(outVars).toEqual({ backgroundCheckPassed: true, riskScore: 42 });
    expect(eventSpy).toHaveBeenCalledWith(expect.objectContaining({ result: 'success' }));
  });

  it('reports BPMN error when background check is attempted without validation', async () => {
    const eventSpy = jest.spyOn(app, 'eventLog').mockResolvedValue(undefined);
    const task = makeTask({ userId: 'userC', validated: false, applicationId: 'dddddddd-dddd-dddd-dddd-dddddddddddd' });
    const taskService = makeTaskService();
    await client.__trigger('onboard-user.run-background-check', { task, taskService });
    expect(taskService.handleBpmnError).toHaveBeenCalledTimes(1);
    expect(taskService.complete).not.toHaveBeenCalled();
  ...
  it('handles technical failure during background check', async () => {
    ...
    const task = makeTask({ userId: 'userD', validated: true, applicationId: 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee' });
    const taskService = makeTaskService();
    await client.__trigger('onboard-user.run-background-check', { task, taskService });
    expect(taskService.handleFailure).toHaveBeenCalledTimes(1);
    expect(taskService.complete).not.toHaveBeenCalled();
    expect(taskService.handleBpmnError).not.toHaveBeenCalled();
    expect(eventSpy).toHaveBeenCalledWith(expect.objectContaining({ result: 'failure' }));
  });

  it('completes onboarding when previous steps succeed', async () => {
    const eventSpy = jest.spyOn(app, 'eventLog').mockResolvedValue(undefined);
    // HTTP returns success for onboarding API.
    (http.post as jest.Mock).mockResolvedValueOnce({ body: { onboarded: true } });
    const task = makeTask({
      userId: 'userE',
      validated: true,
    ...
    const taskService = makeTaskService();
    await client.__trigger('onboard-user.call-onboarding-api', { task, taskService });
    expect(taskService.complete).toHaveBeenCalledTimes(1);
    const outVars = (taskService.complete.mock.calls[0][1] as any).getAll();
    expect(outVars).toEqual({ onboarded: true, reason: undefined });
    expect(eventSpy).toHaveBeenCalledWith(expect.objectContaining({ result: 'success' }));
  });
  ...
  it('returns onboarding negative result without calling API when background check failed', async () => {
    ...
    expect(taskService.complete).toHaveBeenCalledTimes(1);
    const outVars = (taskService.complete.mock.calls[0][1] as any).getAll();
    expect(outVars).toEqual({ onboarded: false, reason: 'Background check failed' });
  ...
  it('handles technical failure during onboarding API call', async () => {
    const eventSpy = jest.spyOn(app, 'eventLog').mockResolvedValue(undefined);
    (http.post as jest.Mock).mockRejectedValueOnce(new Error('downstream unavailable'));
    const task = makeTask({
      userId: 'userH',
      validated: true,
      backgroundCheckPassed: true,
